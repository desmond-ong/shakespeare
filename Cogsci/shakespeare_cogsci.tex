...
Second, are emotion-caused actions---specifically, emotional expressions---independent of beliefs and desires in the intuitive theory? Intuitively, this seems unlikely. For one, emotional display rules dictate what expressions are more appropriate in some contexts rather than other \cite<e.g., >{Matsumoto1990}. Display rules also subsume prosocial or politeness considerations: presumably, one might want to hide their joy from a recent promotion when comforting a friend who has just lost their dog, or force a laugh at a boss' joke. 
Second, crocodile tears and other deliberately deceptive strategies \cite<e.g., >{Depaulo2003} are a common, if distasteful, intentional use of emotional expressions as a means to obtain some desired outcome. 
%but two of the top-down cognitive processes that influence ``automatic" emotional expressions. 
Figure \ref{ModelsOfBehaviorFig}c illustrates this possibility by allowing an agent's beliefs and desires to influence their emotional expressions. Indeed, emotional expressions seem to be this interesting category of behavior that do not fall simply into the ``Intentional" vs. ``Unintentional" divide, as there are clear examples of both. Furthermore, under this model, beliefs and desires can impact expressions (thus, they do not fall into ``Unintentional"), but at the same time are not required for expressions (thus, they do not fall into ``Intentional" either). Emotional expressions form an interesting class whose place in the lay theory deserves further attention, and Figure \ref{ModelsOfBehaviorFig}c provides one interpretation.
